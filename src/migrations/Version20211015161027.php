...
function setVA(mixed $wannd, mixed $wannt, mixed $was, mixed $wo, mixed $imgid = null, mixed $videolink = null, mixed $style = null): void
{
    ...
    if (is_string($was)) { // Wenn noch ganz alte Schreibweise, in Bestandteile zerlegen.
        $was = $migration->SplitupOldTitel($was);
    }

    list($str_start_date, $str_end_date) = $migration->GetStartEndDateAsString($wannd);
    list($str_start_time, $str_end_time) = $migration->GetStartEndTimeAsString($wannt);

    $DateTimeSQL_start = $migration->GetStartDateTimeSQL($str_start_date, $str_start_time);
    $DateTimeSQL_end   = $migration->GetEndDateTimeSQL($str_end_date, $str_end_time, $DateTimeSQL_start);

    $migration->addInsert(
        new Event(
            titel:    utf8_encode($was['titel']),
            subtitel: utf8_encode($was['subtitel']),
            start:    (string) $DateTimeSQL_start,
            end:      (string) $DateTimeSQL_end,
            ort:      $wo,
            link:     $was['link']
        )
    );
